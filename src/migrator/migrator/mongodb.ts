...
import { MongoClient, type ObjectId } from "../../shared/deps/db.ts";
...
class MongoDbMigrator implements Migrator {
  ...
  async runMigrations(): Promise<void> {
    try {
      this.logger.info(
        `Running MongoDB migrations from folder '${this.migrationsFolderPath}'…`
      );
      ...
      const pendingMigrationFiles = migrationFiles.filter(
        (mf) => !migrations.some((m) => m.fileName === mf.name)
      );
      ...
      this.logger.info(
        `Discovered ${pendingMigrationFiles.length} pending migration(s)`
      );
      ...
      for (const pendingMigrationFile of pendingMigrationFiles) {
        if (migrations.some((x) => x.fileName === pendingMigrationFile.name)) {
          this.logger.info(
            `Skipping migration file '${pendingMigrationFile.name}' because it's already applied.`
          );
          continue;
        }
        ...
        if (!migration["up"]) {
          this.logger.warn(
            `Skipping migration file '${pendingMigrationFile.name}' because it lacks the 'up' function.`
          );
          continue;
        }
        this.logger.info(
          `Applying migrations from file '${pendingMigrationFile.name}'…`
        );
