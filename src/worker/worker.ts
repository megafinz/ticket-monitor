  ...
  while (!abortSignal.aborted) {
    try {
      ...
      if (requests.length > 0) {
        logger.info(
          `Processing ${requests.length} ticket monitoring request(s)…`
        );
        for (const request of requests) {
          logger.info("--------------------------------------------");
          logger.info(
            `Processing request '${request.title}' with URL '${request.pageUrl}'…`
          );
          ...
          switch (result.type) {
            ...
            case "request-expired":
              logger.info(
                `Ticket monitoring request '${request.title}' has expired`
              );
              logger.info(`Removing request '${request.title}' from database…`);
              await db.removeRequest(request);
              break;
